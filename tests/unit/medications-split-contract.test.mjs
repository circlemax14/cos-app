...
test('headings only appear when BOTH kinds are present', () => {
  // A lone "MEDICAL" heading over a list that is entirely medical tells the
  // patient nothing and just adds furniture.
  assert.match(SECTION, /const showHeadings = medical\.length > 0 && psychiatric\.length > 0/);
});

...
test('(a) intake-report-builder.ts declares IMMUNIZATIONS_EHR_ENABLED = false as const at module scope', () => {
  assert.match(
    ...
    'intake-report-builder.ts must declare `export const IMMUNIZATIONS_EHR_ENABLED = false as const` at module scope (COS-481 Phase 2 dark-launch kill switch). If this fails, either (i) the constant was renamed/removed — the FE lost its one-line OTA-revert lever, (ii) the default drifted to `true` — the EHR layer ships enabled to every user on the next OTA without Ken\'s approval, (iii) `as const` was dropped — the type widens to `boolean` and future runtime toggling can slip in without a wire like this catching it, or (iv) the RHS was promoted to `process.env.X === "true"` / a hook lookup — silently resolves to false in stages that don\'t set the var and prevents the intended flip. Do NOT flip this wire to accept the mutated shape without an explicit change comment on the const.',
  );
});

test('(a-drift) IMMUNIZATIONS_EHR_ENABLED RHS is the literal boolean `false`', () => {
  ...
  const rhs = m[1].trim();
  assert.ok(
    rhs === 'false' || /^false\s+as\s+const$/.test(rhs),
    `IMMUNIZATIONS_EHR_ENABLED must be the boolean literal \`false\` (optionally with \`as const\`) per COS-481 Phase 2 dark-launch spec. Actual RHS: ${JSON.stringify(rhs)}. Common drift shapes that trip this wire: \`true\` (feature enabled without approval), \`process.env.X === 'true'\` (silently resolves to false in stages), \`Boolean(x)\` (widens the type). Revert to \`= false as const\` unless Ken has explicitly approved a flip.`,
  );
});

...
import { localDayIso, todayLocalIso, dayKeyOf, tzOffsetMinutes, eventDayKey, formatDayLabel } from './day-key.ts';
...
// ── eventDayKey: calendar events, which come in two flavours ─────────

test('a UTC instant maps to the LOCAL day it falls on', () => {
  // Synthesised events (virtualEventFromAppEntity, toIso, the undated anchor)
  // are `Z`. Slicing those gives the UTC day, so an evening appointment files
  // under tomorrow for anyone west of UTC — all year, not just in a window.
  const evening = new Date(2026, 7, 12, 19, 0); // 7pm local
  const asZ = evening.toISOString();
  assert.equal(eventDayKey({ startDate: asZ }), '2026-08-12');
});

test('an offset-carrying string still resolves to the right local day', () => {
  // expo-calendar strings pass through isoOf verbatim and keep their offset.
  assert.equal(
    eventDayKey({ startDate: '2026-08-03T08:15:00.000-0700' }),
    localDayIso(new Date('2026-08-03T08:15:00.000-0700')),
  );
});

test('ALL-DAY events keep the date as given', () => {
  // The reason this is a helper and not a find-and-replace. An all-day event
  // is anchored to a DATE; converting it to a local instant shifts it to the
  // previous day for anyone west of UTC.
  assert.equal(
    eventDayKey({ startDate: '2026-08-12T00:00:00.000Z', allDay: true }),
    '2026-08-12',
  );
});

test('a malformed date falls back rather than returning null', () => {
  // Callers use this as a Map key and to compare against a selected day; null
  // would silently group every broken event together.
  assert.equal(eventDayKey({ startDate: 'not-a-date' }), 'not-a-date'.slice(0, 10));
});

// ── formatDayLabel — Ken 2026-08-14, "put today's date on schedule" ──

test('renders a readable date', () => {
  // Pinned to en-GB so the assertion does not depend on the machine locale.
  assert.equal(formatDayLabel('2026-08-14', 'en-GB'), 'Friday 14 August');
  assert.equal(formatDayLabel('2026-01-05', 'en-GB'), 'Monday 5 January');
});

test('is built from the KEY, so it cannot disagree with the day shown', () => {
  // The label and the schedule must roll over together. Deriving the label
  // from new Date() instead would let them drift apart at midnight — the exact
  // class of bug this module exists to end.
  assert.equal(formatDayLabel('2026-12-31', 'en-GB'), 'Thursday 31 December');
});

test('falls back to the raw key rather than throwing', () => {
  // A schedule without a date is a small loss; one that crashes is not.
  for (const bad of ['', 'today', '2026-13-45', 'not-a-date']) {
    assert.equal(typeof formatDayLabel(bad, 'en-GB'), 'string');
  }
  assert.equal(formatDayLabel('today', 'en-GB'), 'today');
});

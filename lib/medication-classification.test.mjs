/**
 * Medical vs Psychotropic medication grouping (SCRUM-674a).
 *
 * The asymmetry drives every test here: an antipsychotic under "Medical" is a
 * missed grouping; lisinopril under "Psychotropic" is the app asserting
...
test('common psychotropic medications are named, by generic AND brand', () => {
  const PSYCH = [
  ...
  for (const n of PSYCH) {
    assert.equal(c(n), 'psychotropic', `${n} should be psychotropic`);
  }
});

test('THE ERROR THAT MATTERS: ordinary medications are never called psychotropic', () => {
  const MEDICAL = [
  ...
  for (const n of MEDICAL) {
    assert.equal(c(n), 'medical', `${n} must NOT be called psychotropic`);
  }
});

test('dual-indication drugs are left as medical, deliberately', () => {
  // Each has a common non-psychotropic use. Asserting the psychotropic one from
...
test('mood stabilisers are NOT psychotropic by default, but can be flipped', () => {
  // ATC N03. Whether they read as psychotropic depends on the indication, which
  ...
  for (const n of ['Lamotrigine', 'Lamictal', 'Divalproex', 'Depakote', 'Carbamazepine']) {
    ...
    assert.equal(
      classifyMedication({ name: n }, { treatMoodStabilisersAsPsychotropic: true }),
      'psychotropic',
...
test('dose, form and strength do not defeat matching', () => {
  for (const n of [
    ...
    assert.equal(c(n), 'psychotropic', `${n} should still match`);
  }
});
...
test('junk input is medical, never psychotropic', () => {
...
test('splitting preserves order within each bucket and drops nothing', () => {
  ...
  const { medical, psychotropic } = splitByMedicationClass(meds);
  assert.deepEqual(medical.map((m) => m.name), ['Lisinopril', 'Metformin', 'Aspirin']);
  assert.deepEqual(psychotropic.map((m) => m.name), ['Sertraline', 'Quetiapine']);
  assert.equal(medical.length + psychotropic.length, meds.length, 'nothing dropped');
});

test('an empty list splits into two empty lists', () => {
  assert.deepEqual(splitByMedicationClass([]), { medical: [], psychotropic: [] });
});

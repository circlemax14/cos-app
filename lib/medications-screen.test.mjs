 ...
 * looking finished:
 *
 *   1. the medical/psychotropic split was TWO HEADED SECTIONS, not icons
...
test('ONLY PSYCHIATRIC IS MARKED — medical is a default, not a finding', () => {
  ...
  // list, including psychotropic drugs it does not know. Badging that would
  // present a fallback as a conclusion, on a screen a patient may hand to a
  // family member.
  assert.match(MEDS, /classMark\(medClass\)/)
  assert.match(MEDS, /isPsychRow \? \{ borderLeftWidth: 3/, 'psychotropic rows get an edge')
...
test('the legend only appears when there is something to tell apart', () => {
  ...
  assert.match(MEDS, /const showLegend = medical\.length > 0 && psychotropic\.length > 0/)
})

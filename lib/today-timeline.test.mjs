...
test('nothing due yet is 100%, not 0%', () => {
  ...
  //
  // Uses its own fixture rather than `day`: `day` has three tasks already
  // ticked off, so since the 2026-08-14 fix ("a done task is always due")
  // nothing-due-yet is not true of it at 6am — three tasks are due and done.
  // That is the correct behaviour, and it made this test's name a lie. The
  // claim being made here is about a genuinely empty morning.
  const a = computeAdherence(
    [
      item({ title: 'Meditation', time: '13:00', done: false }),
      item({ title: 'Prayer', time: '17:00', done: false }),
    ],
    6 * 60,
  );
...
test('the old `day` fixture at 6am now reads 3/3, because those three are done', () => {
  // Pinning the behaviour change the test above used to mask.
...
test("KEN'S BUG: completing an evening task at breakfast shows a percentage, not 'Nothing due yet'", () => {
  // Reported 2026-08-14: "still seeing nothing due yet rather than percentage
  // even after completing task."
  //
  // At 08:00 nothing is due by the clock. Tick off the 20:00 task and the old
  // rule still reported due=0, so the header rendered "Nothing due yet" — the
  // app looked like it had ignored the tap.
  const a = computeAdherence(
    [
      item({ title: 'Evening walk', time: '20:00', done: true }),
      item({ title: 'Night meds', time: '22:00', done: false }),
    ],
    8 * 60,
  );
  assert.equal(a.due, 1, 'the completed task must be counted as due');
  assert.equal(a.done, 1);
  assert.equal(a.percent, 100);
});

test('an UNDONE future task still does not drag the number down', () => {
  // The other half of the same rule. If `done` short-circuited the clock test
  // for every task, an untouched evening task would land in the denominator at
  // breakfast and the score would read as failure all morning — the exact
  // problem the "due so far" design exists to avoid.
  const a = computeAdherence(
    [
      item({ title: 'Morning meds', time: '07:00', done: true }),
      item({ title: 'Evening walk', time: '20:00', done: false }),
    ],
    8 * 60,
  );
  assert.equal(a.due, 1, 'only the morning task is owed yet');
  assert.equal(a.done, 1);
  assert.equal(a.percent, 100);
  assert.equal(a.total, 2, 'and the evening one is still "to come"');
});

test('a completed future task is no longer counted as "still to come"', () => {
  // stillToCome is rendered as total - due in AdherenceScore. A done task
  // sitting in both `due` and "still to come" would be double-counted copy.
  const a = computeAdherence(
    [
      item({ title: 'Evening walk', time: '20:00', done: true }),
      item({ title: 'Night meds', time: '22:00', done: false }),
  ...
  assert.equal(a.total - a.due, 1, 'only the undone night task is still to come');
});

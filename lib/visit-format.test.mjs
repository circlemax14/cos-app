...
const SCREEN = read('app', 'Home', 'agency-detail.tsx')
...
test('it renders nothing rather than an empty or error state', () => {
  ...
  //
  // COS-996 narrowed ONE of these three and deliberately left the other two.
  // As a tab (showEmptyState) a successful fetch with zero rows now explains
  // itself, because a patient who tapped "Scheduling" and got a blank pane
  // reads it as broken. Loading and ERROR still render nothing: saying "no
  // visits are booked" when the request FAILED asserts something we do not
  // know, and a patient with a visit tomorrow would act on it.
  assert.match(CARD, /if \(isLoading\) return null/)
  assert.match(CARD, /if \(isError\) return null/)
  assert.match(CARD, /if \(visits\.length === 0\) return empty/)
  assert.match(CARD, /showEmptyState/, 'the empty state must be opt-in, not the default')
})
...
test('the schedule is a tab beside the team, inside the approved branch', () => {
  /*
   * This used to assert the visits card rendered BELOW the team list, so that
   * "when" never preceded "who". COS-999 made them two TABS, so ordering on the
   * page is no longer the thing that separates them — the tab does — and the
   * four-row card was replaced by AgencyScheduleCalendar.
   *
   * What has NOT changed, and is the half worth keeping: neither may render
   * outside the approved branch, or a patient with no agency is shown an
   * agency's staff and schedule. And the team stays the DEFAULT tab, because
   * "who looks after me" is the question people open this screen with.
   */
  const team = SCREEN.indexOf('<AgencyTeamSection')
  const schedule = SCREEN.indexOf('<AgencyScheduleCalendar')
  assert.ok(team > 0, 'the team section must be mounted')
  assert.ok(schedule > 0, 'the schedule calendar must be mounted')
  assert.match(SCREEN, /useState<'team' \| 'scheduling'>\('team'\)/,
    'the team tab must be the default')

  // Both must sit inside the requestStatus === 'approved' branch.
  const approved = SCREEN.indexOf("requestStatus === 'approved'")
  const pending = SCREEN.indexOf("requestStatus === 'pending'")
  assert.ok(approved > 0 && approved < team && approved < schedule,
    'both must render inside the approved branch')
  assert.ok(team < pending && schedule < pending,
    'both must close before the pending branch begins')
})

...
test('ordering is delegated to tested logic, not inlined', () => {
  // Ordering moved into orderMarkers() in lib/health-age-presentation once the
  // rule got real — readings first by magnitude, empties last. An inline sort
  // in a 1200-line screen is not something anyone can test.
  const fn = SRC.slice(SRC.indexOf('function ContributorCards'))
  assert.match(fn, /orderMarkers\(/)
})

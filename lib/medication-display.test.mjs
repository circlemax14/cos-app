...
test('THE CONFIDENCE ONE: only psychotropic is asserted', () => {
  // classifyMedication returns 'medical' for anything not on its curated
  // list, INCLUDING psychotropic drugs it does not know. Marking that as a
  // finding would have the app claim "this is medical" when what it knows is
  // "this is not in my psychotropic list".
  assert.equal(classMark('psychotropic').show, true)
  assert.equal(classMark('psychotropic').label, 'psychotropic')
